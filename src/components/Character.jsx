      ...
      <div className="flex space-x-2 mb-4">
        {Object.keys(CLASS_LIST).map(className => (
          <button
            ...
            className={`px-4 py-2 rounded ${Object.entries(CLASS_LIST[className]).every(([attr, min]) => character.attributes[attr] >= min) ? 'bg-green-500 text-white' : 'bg-gray-500 text-white'}`}
          >
            {className}
          </button>
        ))}
